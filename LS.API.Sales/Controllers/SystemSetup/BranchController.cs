...
namespace LS.API.Sales.Controllers
{
    public class BranchController : BaseController
    {
        ...
        [HttpGet("getSelectBranchNameCodeList")]
        public async Task<IActionResult> GetSelectBranchNameCodeList()
        {
            var obj = await Mediator.Send(new GetSelectBranchNameCodeList() { User = UserInfo() });
            return obj is not null ? Ok(obj) : NotFound(new ApiMessageDto { Message = ApiMessageInfo.NotFound });
        }
        [HttpGet("getSelectSysAccountBranchList")]
        public async Task<IActionResult> GetSelectSysAccountBranchList()
        {
            var obj = await Mediator.Send(new GetSelectSysAccountBranchList() { User = UserInfo() });
            return obj is not null ? Ok(obj) : NotFound(new ApiMessageDto { Message = ApiMessageInfo.NotFound });
        }

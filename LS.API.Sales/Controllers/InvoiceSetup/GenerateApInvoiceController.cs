...
namespace LS.API.Sales.Controllers
{
    public class GenerateApInvoiceController : FileBaseController
    {
        ...
        public async Task<IActionResult> Get([FromQuery] PaginationFilterDto filter)
        {
            ...
            var list = await Mediator.Send(new GetApSalesInvoiceList()
            {
                Input = filter.Values(),
        ...
        [HttpGet("getPurchaseInvoicePrintingList")]
        public async Task<ActionResult> GetPurchaseInvoicePrintingList([FromQuery] string branchCode, [FromQuery] DateTime? from, [FromQuery] DateTime? to, [FromQuery] int? id)
        {
            var result = await Mediator.Send(new GetPurchaseInvoicePrintingList() { DateFrom = from, DateTo = to, BranchCode = branchCode, Id = id, User = UserInfo() });
            return Ok(result);
        }

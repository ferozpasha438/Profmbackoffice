...
namespace LS.API.FomMobB2C.Controllers
{
    ...
    public class BaseController : ApiControllerBase
    {
        ...
        public string Mobile => HasClaims() ? Convert.ToString(GetCliams().FirstOrDefault(c => c.Type == "mobile")?.Value ?? string.Empty) : string.Empty;
        public string LoginType => HasClaims() ? Convert.ToString(GetCliams().FirstOrDefault(c => c.Type == "logintype")?.Value ?? string.Empty) : string.Empty;
        public string Role => HasClaims() ? Convert.ToString(GetCliams().FirstOrDefault(c => c.Type == "role")?.Value ?? string.Empty) : string.Empty;

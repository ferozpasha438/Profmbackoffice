...
namespace LS.API.Controllers.FInanceMgt
{
    public class BatchSetupController : BaseController
    {
        ...
        public async Task<IActionResult> Get()
        {
            ...
            var list = await Mediator.Send(new GetBatchSetupList() { User = UserInfo() });
            return Ok(list);
        }
        ...
        {
            var obj = await Mediator.Send(new GetSingleBatchSetup() { Id = id, User = UserInfo() });
            return obj is not null ? Ok(obj) : NotFound(new ApiMessageDto { Message = ApiMessageInfo.NotFound });
        }

        [HttpPost]
        public async Task<ActionResult> Create([FromBody] TblFinSysBatchSetupDto input)
        {
            //await Task.Delay(3000);

            var seg = await Mediator.Send(new CreateUpdateBatchSetup() { Input = input, User = UserInfo() });
            if (seg.Id > 0)
            {
                if (input.Id > 0)
                    return NoContent();

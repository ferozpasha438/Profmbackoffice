...
namespace LS.API.Purchase.Controllers
{
    public class VendorController : BaseController
    {
        ...
        public async Task<IActionResult> GetLanVendorsCustomList([FromQuery] bool? isPayment)
        {
            var obj = await Mediator.Send(new GetLanVendorsCustomList() { IsPayment = isPayment, User = UserInfo() });
            return obj is not null ? Ok(obj) : NotFound(new ApiMessageDto { Message = ApiMessageInfo.NotFound });
        }

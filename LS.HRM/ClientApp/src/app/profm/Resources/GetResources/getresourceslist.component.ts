...
import { AddupdateresourcesComponent } from '../Sharedpages/addupdateresources.component';
import { ParentFomMgtComponent } from '../../../sharedcomponent/parentfommgt.component';

@Component({
  selector: 'app-getresourceslist',
  templateUrl: './getresourceslist.component.html',
  styles: [
  ]
})
export class GetresourceslistComponent extends ParentFomMgtComponent implements OnInit {
  ...
  displayedColumns: string[] = ['resTypeCode', 'nameEng', 'nameAr', 'createdDate', 'approvalAuth', 'isActive','Actions'];
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, this.translate.instant('Add_Resources'), 'Add');
  }
  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, this.translate.instant('Update_Resources'), 'Update');
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('Resource', id).subscribe(res => {
          this.refresh();

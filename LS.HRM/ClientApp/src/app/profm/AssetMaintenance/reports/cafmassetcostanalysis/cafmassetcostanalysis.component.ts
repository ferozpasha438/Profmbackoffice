...
@Component({
  selector: 'app-cafmassetcostanalysis',
  templateUrl: './cafmassetcostanalysis.component.html',
  styles: [
  ]
})
export class CafmassetcostanalysisComponent extends ParentB2CFrontComponent implements OnInit {
  ...
  search() {
    ...
    this.isLoading = true;
    this.apiService.getall(`assetMaintenanceReport/cafmassetcostanalysis?customerCode=${this.customerCode}&projCode=${this.projCode}&from=${this.utilService.getCommonDate(this.dateFrom)}&to=${this.utilService.getCommonDate(this.dateTo)}`).subscribe(res => {

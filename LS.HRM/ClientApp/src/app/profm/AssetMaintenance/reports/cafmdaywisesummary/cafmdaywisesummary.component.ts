...
@Component({
  selector: 'app-cafmdaywisesummary',
  templateUrl: './cafmdaywisesummary.component.html',
  styles: [
  ]
})
export class CafmdaywisesummaryComponent extends ParentB2CFrontComponent implements OnInit {
  ...
  search() {
    ...
    if (this.dateFrom && this.dateTo) {
      this.isLoading = true;
      this.apiService.getall(`assetMaintenanceReport/cafmDayWiseSummary?customerCode=${this.customerCode}&projCode=${this.projCode}&from=${this.utilService.getCommonDate(this.dateFrom)}&to=${this.utilService.getCommonDate(this.dateTo)}`).subscribe(res => {

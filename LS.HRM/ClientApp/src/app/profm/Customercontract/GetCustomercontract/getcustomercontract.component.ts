...
import { AddupdatecustomercontractComponent } from '../Sharedpages/addupdatecustomercontract.component';
import { ParentFomMgtComponent } from '../../../sharedcomponent/parentfommgt.component';
import { GetschedulesComponent } from '../Sharedpages/getschedules.component';
import { GetschedulelistComponent } from '../Sharedpages/getschedulelist.component';
...
export class GetcustomercontractComponent extends ParentFomMgtComponent implements OnInit {
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('FomCustomerContract', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdatecustomercontractComponent);
  ...
  private openDialogManageRow(row: any, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
  ...
  private openGenSchDetails(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
  ...
  public genSchDetails(row: any) {
    this.openGenSchDetailsRow(row, DBOperation.update, this.translate.instant('Schedule_Contract_Details'), 'Schedule_Details');
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('FomCustomerContract', id).subscribe(res => {
          this.refresh();

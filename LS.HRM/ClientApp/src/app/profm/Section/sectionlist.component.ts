...
import { AddupdatesectionComponent } from './addupdatesection.component';
...
export class SectionlistComponent extends ParentB2CFrontComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sectionCode', 'nameEng', 'nameArabic', 'description','forAssetMgt', 'createdDate', 'isActive', 'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    ...
    this.apiService.getPagination('fomSection', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, 'Add_New_Section', 'Add', AddupdatesectionComponent);

  }
  public edit(id: number) {   
    this.openDialogManage(id, DBOperation.update, 'Update_Section', 'Update', AddupdatesectionComponent);
  }
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('fomSection/deletesection', id).subscribe(res => {
          this.refresh();

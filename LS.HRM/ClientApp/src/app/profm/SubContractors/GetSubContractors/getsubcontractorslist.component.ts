...
import { AddupdatesubcontractorsComponent } from '../Sharedpages/addupdatesubcontractors.component';
import { ParentFomMgtComponent } from '../../../sharedcomponent/parentfommgt.component';

@Component({
  selector: 'app-getsubcontractorslist',
  templateUrl: './getsubcontractorslist.component.html',
  styles: [
  ]
})
export class GetsubcontractorslistComponent extends ParentFomMgtComponent implements OnInit {
  ...
  displayedColumns: string[] = ['deptCodes', 'subContCode', 'nameEng', 'nameArabic', 'createdDate', 'isActive', 'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    ...
    this.apiService.getPagination('fomSubContractor', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdatesubcontractorsComponent);
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('fomSubContractor', id).subscribe(res => {
          this.refresh();

...
import { GetscheduleslistdatesComponent } from '../getscheduleslistdates.component';

@Component({
  selector: 'app-allschedulecalendar',
  templateUrl: './allschedulecalendar.component.html',
})
export class AllschedulecalendarComponent extends ParentFomMgtComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private apiService: ApiService, private translate: TranslateService,
    private authService: AuthorizeService, private utilService: UtilityService, public dialogRef: MatDialogRef<GetscheduleslistdatesComponent>,
  ...
  getCurrentAndLastDateOfMonth() {
    ...
    if (this.selectedDate) {
      this.scheduleData = [];
      this.scheduleDays = [];
      this.apiService.getall(`FomCustomerContract/AllFomCalenderScheduleList`).subscribe(result => {

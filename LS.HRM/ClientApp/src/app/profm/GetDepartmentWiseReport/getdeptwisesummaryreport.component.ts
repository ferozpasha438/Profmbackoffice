...
@Component({
  selector: 'app-getdeptwisesummaryreport',
  templateUrl: './getdeptwisesummaryreport.component.html',
  styleUrls: ['./getdeptwisesummaryreport.component.scss']
})
export class GetdeptwisesummaryreportComponent extends ParentFomMgtComponent implements OnInit {
  ...
  displayedColumns: string[] = ['deptName', 'opening', 'received', 'totReceive', 'foreClosed', 'closed', 'completed', 'totalClosed', 'totJobs', 'closing', 'percentage'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    ...
    this.apiService.getPagination('FomCustomerContract/getDeptWiseSummaryJobTicketsReport', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
  ...
  //createChart() {
  //  const completed = 10; // Replace with your data
  //  const balance = 19;   // Replace with your data
  ...
  //      datasets: [
  //        {
  //          data: [completed, balance],
  //          backgroundColor: ['#4caf50', '#ff9800'], // Green and Orange
  //        },
  //      ],
  //    },
  //    options: {
  ...
  //              const data = tooltipItem.dataset.data as number[]; // Cast dataset data to number array
  //              const total = data.reduce((acc, value) => acc + value, 0);
  //              const value = tooltipItem.raw as number; // Cast raw value to number
  ...
  renderPerformanceChart(): void {
    ...
    this.chartInstance = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          data: [this.balancePercentage, this.completedPercentage], // Bind percentages here  [data.balancePercentage, data.completedPercentage]
          backgroundColor: ['#FF6384', '#36A2EB'] // Colors for Balance and Completed
        }]
      },
      options: {
        responsive: true,
  ...
  loadData() {
    ...
    this.isLoading = true;
    this.apiService.postFomCpUrl('FomCustomerContract/getDeptWiseSummaryJobTicketsReport', this.filter)
      .subscribe(result => {
        this.isLoading = false;

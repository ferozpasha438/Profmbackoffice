...
import { AddupdateitemcategoriesComponent } from '../Sharedpages/addupdateitemcategories.component';

@Component({
  selector: 'app-getitemcategories',
  templateUrl: './getitemcategories.component.html',
  styles: [
  ]
})
export class GetitemcategoriesComponent extends ParentFomMgtComponent implements OnInit {

  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;

  displayedColumns: string[] = ['itemCatCode', 'itemCatName','itemCatName_Ar', 'itemCatDesc', 'itemCatPrefix', 'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    ...
    this.apiService.getPagination('fomItemCategory', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateitemcategoriesComponent);
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('fomItemCategory', id).subscribe(res => {
          this.refresh();

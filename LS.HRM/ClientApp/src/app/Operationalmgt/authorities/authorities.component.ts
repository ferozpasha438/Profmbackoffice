...
import { AddUpdateAuthoritiesComponent } from './add-update-authorities/add-update-authorities.component';
...
export class AuthoritiesComponent
  ...
  displayedColumns: string[] = ['branchCode','appAuth','userName', 'canApproveEnquiry',
    'canApproveSurvey', 'canApproveEstimation', 'canApproveProposal', 'canApproveContract', 'canModifyEstimation','canConvertEnqToProject',
    'canConvertEstimationToProposal', 'canConvertProposalToContract', 'canCreateRoaster', 'canEditRoaster', 'canEditEnquiry', 'canAddSurveyorToEnquiry','canApprovePvReq',/*'isActive',*/ 'Actions'];
  ...
  private loadServices(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('OpAuthorities/getAuthoritiesPagedList', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('OpAuthorities',id).subscribe(res => {

...
@Component({
  selector: 'app-create-update-transfer-with-replacement',
  templateUrl: './create-update-transfer-with-replacement.component.html'
})
export class CreateUpdateTransferWithReplacementComponent extends ParentOptMgtComponent implements OnInit {
  modalTitle: string;
  ...
  constructor(public datepipe: DatePipe, private translate: TranslateService, private fb: FormBuilder, private authService: AuthorizeService, private utilService: UtilityService, private apiService: ApiService, public dialogRef: MatDialogRef<CreateUpdateTransferWithReplacementComponent>) {
  ...
  editForm() {
    ...
    this.apiService.get('PvTransferWithReplacement/getPvTransferWithReplacementReqById', this.id).subscribe(res => {
  ...
  submit() {
    if (this.form.valid) {
      ...
      this.apiService.post('PvTransferWithReplacement', this.form.value)
        .subscribe(res => {
          if (res) {

...
import { CreateUpdateTransferWithReplacementComponent } from './create-update-transfer-with-replacement/create-update-transfer-with-replacement.component';
@Component({
  selector: 'app-transfer-with-replacement',
  templateUrl: './transfer-with-replacement.component.html'
})
export class TransferWithReplacementComponent extends ParentOptMgtComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  displayedColumns: string[] = ['srcCustomerCode', 'srcSiteCode', 'srcProjectCode', 'destCustomerCode', 'destSiteCode', 'destProjectCode', 'srcEmployeeNumber','destEmployeeNumber', 'fromDate', 'isApproved',/* 'isMerged',*/ 'Actions'];
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('PvTransferWithReplacement/getPvTransferWithReplacementReqsPagedList', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, 'New_Request_For_Transfer_Resource', 'Add', CreateUpdateTransferWithReplacementComponent);
  }

  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, 'Updating_Request_For_Transfer_Resource', 'Update', CreateUpdateTransferWithReplacementComponent);
  }
  ...
  public viewRequest(id: number) {
    this.openDialogManage(id, DBOperation.create, 'Request_For_Transfer_Employee', 'View', CreateUpdateTransferWithReplacementComponent);
  }

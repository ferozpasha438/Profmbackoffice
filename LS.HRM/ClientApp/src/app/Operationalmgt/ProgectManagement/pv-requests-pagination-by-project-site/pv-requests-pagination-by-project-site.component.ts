...
@Component({
  selector: 'app-pv-requests-pagination-by-project-site',
  templateUrl: './pv-requests-pagination-by-project-site.component.html'
})
export class PvRequestsPaginationByProjectSiteComponent extends ParentOptMgtComponent implements OnInit {
 ...
 approvedStatus: string = "";
  constructor(public datepipe: DatePipe, private fb: FormBuilder, private http: HttpClient, private router: Router, private apiService: ApiService, private authService: AuthorizeService,
    ...
    public pageService: PaginationService, private oprService: OprServicesService, private translate: TranslateService, public dialogRef: MatDialogRef<PvRequestsPaginationByProjectSiteComponent>) {
    super(authService);
  }
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined, approval: string = "", listType: string | null | undefined) {
    ...
    this.apiService.getPagination(`PvAllRequests/getPvAllRequestsPagedListByProjectSite/${this.project.projectCode}/${this.project.siteCode}`, this.utilService.getOprQueryString(page, pageCount, query, orderBy, approval,'',0,'', listType)).subscribe(result => {
  ...
  private createNewWindow(id: 0, projectData:any, dbops: DBOperation, modalTitle: string, modalBtnTitle: string, Component: any) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, Component);
    (dialogRef.componentInstance as any).dbops = dbops;
    (dialogRef.componentInstance as any).modalTitle = modalTitle;
    (dialogRef.componentInstance as any).modalBtnTitle = modalBtnTitle;
    (dialogRef.componentInstance as any).id = id;
    (dialogRef.componentInstance as any).projectData = projectData;
      ...
      if (requestType == "AddResource") {
        ...
        if (requestType == "TransferResource") {
          ...
          if (requestType == "ReplaceResource") {
            ...
            if (requestType == "TransferWithReplacement") {
              this.openDialogManage(id, DBOperation.update, 'Updating_Request_For_Transfer_With_Replacement', 'Update', CreateUpdateTransferWithReplacementComponent);
            }
            else
              if (requestType == "ProjectVariations") {
    ...
    if (requestType == "SwapEmployees") {
      ...
      if (requestType == "RemoveResource") {
        ...
        if (requestType == "AddResource") {
          ...
          this.openDialogManage(row.requestNumber, DBOperation.create, 'Request_For_Add_Resource', 'View',/* PvEmpToResMapComponent*/ CreateUpdateAddResourceComponent);
          ...
          if (requestType == "TransferResource") {
            ...
            if (requestType == "ReplaceResource") {
              this.openDialogManage(row.requestNumber, DBOperation.create, 'Replace_Resource_Request', 'View', CreateUpdateReqReplaceEmployeeComponent);
              ...
              if (requestType == "ProjectVariations") {
                ...
                // this.apiService.get('PVTransferWithReplacement/getTransferWithReplacementReqById', row.requestNumber).subscribe(res => {
  ...
  public createNew() {
    if (!this.isUpdating && !this.isLoading) {
      if (this.listType == "addResource")
        this.createNewWindow(0, this.project, DBOperation.create, 'New_Request_For_Add_Resource', 'Add', CreateUpdateAddResourceComponent);
      else if (this.listType == "removeResource") 
        this.createNewWindow(0, this.project, DBOperation.create, 'New_Request_For_Remove_Resource', 'Add', CreateUpdateReqRemoveResourceComponent);
      else if (this.listType == "replaceResource")
        this.createNewWindow(0, this.project, DBOperation.create, 'New_Request_For_Replace_Employees', 'Add', CreateUpdateReqReplaceEmployeeComponent);

      else if (this.listType == "swapEmployees")
        this.createNewWindow(0, this.project, DBOperation.create, 'New_Request_For_Swap_Employees', 'Add', CreateUpdateSwapEmployeesComponent);
      else if (this.listType == "transferWithReplacement")
        this.createNewWindow(0, this.project, DBOperation.create, 'New_Request_For_Transfer_With_Replacement', 'Add', CreateUpdateTransferWithReplacementComponent);
      else if (this.listType == "transferResource")
        this.createNewWindow(0, this.project, DBOperation.create, 'New_Request_For_Transfer_Employee', 'Add', CreateUpdateTransferResourceReqComponent);

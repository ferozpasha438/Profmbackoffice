...
import { AddupdateCustomerComplaintComponent } from './addupdate-customer-complaint/addupdate-customer-complaint.component';

@Component({
  selector: 'app-customer-complaints',
  templateUrl: './customer-complaints.component.html'
})
export class CustomerComplaintsComponent extends ParentOptMgtComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  displayedColumns: string[] = ['customerCode', 'siteCode', 'projectCode','reasonCodeNameEng', 'reasonCodeNameAr', 'status', 'Actions'];
  ...
  constructor(public datepipe: DatePipe, private fb: FormBuilder, private http: HttpClient, private router: Router, private apiService: ApiService, private authService: AuthorizeService,
    ...
    public pageService: PaginationService, private oprService: OprServicesService, private translate: TranslateService, public dialogRef: MatDialogRef<CustomerComplaintsComponent>) {
    super(authService);
  }
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined, approval: string = "", listType: string | null | undefined) {
    ...
    this.apiService.getPagination(`CustomerComplaints/getCustomerComplaintsPagedListByProjectSite/${this.project.projectCode}/${this.project.siteCode}`, this.utilService.getOprQueryString(page, pageCount, query, orderBy, approval, '', 0, '', listType)).subscribe(result => {
  ...
  delete(row: any) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        ...
        this.apiService.delete('CustomerComplaints', id).subscribe(res => {
  ...
  createUpdate(id: number, action: string) {
    let dialogRef = this.oprService.openAutoHeightWidthDialog(this.dialog, AddupdateCustomerComplaintComponent);
    (dialogRef.componentInstance as any).dbops = DBOperation.create;
    (dialogRef.componentInstance as any).modalTitle = "Customer_Complaint";
  ...
  startAction(row: any) {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res) {
        this.apiService.get('CustomerComplaints/StartAction/', row.id)
          .subscribe(res2 => {
            if (res2) {

...
export class SurveyFormComponent extends ParentOptMgtComponent  implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  displayedColumns: string[] = ['surveyFormCode', 'surveyFormNameArb', 'surveyFormNameEng', 'remarks','Actions'];
  ...
  constructor(private fb: FormBuilder, private http: HttpClient, private router: Router, private apiService: ApiService, private authService: AuthorizeService,
    ...
    public pageService: PaginationService, private oprService: OprServicesService) {
    super(authService);
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('SurveyFormHeader/getSurveyFormHeaderPagedList', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  public create() {
    this.openDialogManage(0, DBOperation.create, 'Adding_New_Survey_Form_Template', 'Add', AddupdateSurveyFormComponent);
  }

  public AttachToService(SurveyFormId: number) {
    this.openDialogManage(SurveyFormId, DBOperation.create, 'Attach_Survey_Form_To_Service', 'Attach', AttachSurveyFormToServiceComponent);

  }


  public edit(id: number) {
    this.openDialogManage(id, DBOperation.update, 'Updating_Survey_Form_Template', 'Update', AddupdateSurveyFormComponent);
  }
  public delete(id: number) {
    const dialogRef = this.utilService.openDeleteConfirmDialog(this.dialog, DeleteConfirmDialogComponent);
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('SurveyFormHeader', id).subscribe(res => {
  ...
  ViewSurveyFormTemplate(rowId:number) {
    //this.oprService.Input = rowId;

    this.openDialogManage(rowId, DBOperation.create, '', '', ViewSurveyFormTemplateComponent);

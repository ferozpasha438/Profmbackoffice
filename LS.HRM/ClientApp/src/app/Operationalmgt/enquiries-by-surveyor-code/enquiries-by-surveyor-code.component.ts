...
@Component({
  selector: 'app-enquiries-by-surveyor-code',
  templateUrl: './enquiries-by-surveyor-code.component.html'
})
export class EnquiriesBySurveyorCodeComponent extends ParentOptMgtComponent implements OnInit {

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  displayedColumns: string[] = ['enquiryNumber', 'enquiryID', 'siteCode', 'serviceCode', 'unitCode', 'pricePerUnit', 'serviceQuantity', 'estimatedPrice','statusEnquiry', 'Actions'];
  ...
  ngOnInit(): void {
      this.surveyorCode = this.oprService.SurveyorCode;
    if (this.surveyorCode == null)
      this.surveyorCode = localStorage.getItem('surveyorCode') as string;
      this.initialLoading();
  }
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    this.isLoading = true;
    this.apiService.getPagination('ServiceEnquiries/getSevriceEnquiriesBySurveyorCodePagedList', this.utilService.getQueryString(page, pageCount, this.surveyorCode, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;

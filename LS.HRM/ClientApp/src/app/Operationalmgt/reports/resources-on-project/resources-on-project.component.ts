...
@Component({
  selector: 'app-resources-on-project',
  templateUrl: './resources-on-project.component.html'
})
export class ResourcesOnProjectComponent extends ParentOptMgtComponent implements OnInit {
  form: FormGroup;
  List: Array<any> = [];
  ...
  search() {
    ...
    this.apiService.post('Reports/getResourcesOnProjectsReports', this.InputQuery).subscribe((res: any) => {
        ...
        if (res != null) {
          ...
          if (this.projectSitesList.length > 0) {
            for (let i = 0; i < this.projectSitesList.length; i += 50) {
              ...
              this.apiService.post('Reports/getResourcesOnProjectsReportsByProjectSites', projectsSites).subscribe((res2) => {

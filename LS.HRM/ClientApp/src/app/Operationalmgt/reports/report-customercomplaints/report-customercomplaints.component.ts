...
@Component({
  selector: 'app-report-customercomplaints',
  templateUrl: './report-customercomplaints.component.html'
})
export class ReportCustomercomplaintsComponent extends ParentOptMgtComponent implements OnInit {
  form: FormGroup;
  List: Array<any> = [];
  ...
  setForm() {
    ...
    this.InputQuery = { fromDate: this.fromDate, todate: this.toDate, customerCode: "", reasonCode: "", branchCode: "", projectCode: "",bookedBy:"" };
  ...
  search() {
    ...
    this.InputQuery = {
      ...
      bookedBy: this.bookedBy == null ? "" : this.bookedBy,

    };

    if (this.fromDate && this.toDate) {
      this.isLoading = true;
      this.apiService.post('Reports/getCustomerComplaintsReport', this.InputQuery).subscribe((res: any) => {
        this.result = res;
  ...
  openPrint() {
    ...
    if (this.result.complaints.length > 0) {
      if (!this.isLoading) {

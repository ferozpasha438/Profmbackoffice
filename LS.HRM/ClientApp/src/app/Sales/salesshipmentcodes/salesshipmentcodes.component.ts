...
@Component({
  selector: 'app-salesshipmentcodes',
  templateUrl: './salesshipmentcodes.component.html',
  styleUrls: [],
})
export class SalesshipmentcodesComponent extends ParentSystemSetupComponent implements OnInit {
  ...
  private loadList(page: number | undefined, pageCount: number | undefined, query: string | null | undefined, orderBy: string | null | undefined) {
    ...
    this.apiService.getPagination('salesShipmentCode', this.utilService.getQueryString(page, pageCount, query, orderBy)).subscribe(result => {
      this.totalItemsCount = 0;
  ...
  private openDialogManage(id: number = 0, dbops: DBOperation, modalTitle: string, modalBtnTitle: string) {
    let dialogRef = this.utilService.openCrudDialog(this.dialog, AddupdateshipmentcodeComponent);
  ...
  public delete(id: number) {
    ...
    dialogRef.afterClosed().subscribe(canDelete => {
      if (canDelete && id > 0) {
        this.apiService.delete('salesShipmentCode', id).subscribe(res => {
          this.refresh();
